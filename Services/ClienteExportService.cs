...
namespace RotoGestionClientes
{
    public class ClienteExportService
    {
        ...
        public void ExportarClienteDeLista(int clienteId, string carpetaDestino)
        {
            string? nombreCliente = _context.Clientes.Where(c => c.Id == clienteId).Select(c => c.Nombre.Trim()).FirstOrDefault();
            
            if (nombreCliente == null) return;

            var fileName = Path.Combine(carpetaDestino,$"{LimpiarNombreFichero(nombreCliente)}.roto");
        ...
        private string LimpiarNombreFichero(string nombre)
        {
